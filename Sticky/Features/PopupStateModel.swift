...
enum PopupStyle {
    ...
    func getMessage() -> Message {
        switch self {
        case .exit:
            return Message(style: .exit, title: "챌린지 종료하기", description: """
            앗! 지금까지의 쌓은 연속기록이 사라집니다!
            정말 챌린지를 종료하시겠어요?
            """, confirmString: "그만 할래요", rejectString: "계속 할게요")
        case .fail:
            return Message(style: .fail, title: "챌린지 종료", description: """
            위치를 이탈하여 챌린지가 종료되었습니다.
            최종 기록을 공유할까요?
            """, confirmString: "네! 공유할래요", rejectString: "안할래요")
        case .outing:
            return Message(style: .outing, title: "외출하기", description: """
